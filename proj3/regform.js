...
// used when you click reset, it should re-focus to the first item
	...
	// used when user click on the link or "a tag"
	$('a').on('click',function() {
		getServerMessage();
		});
...
//alert("IN DUP HANDLER response is "+response);
	if(response == "dup") {
		$('#status').text("THIS IS A DUPLICATE"); }
	else if(response == "OK") {
		$('form').serialize();
		$('form').submit();
	}
...
function getServerMessage() {	
    $.post("hello.php", handleData);
	}
	
function handleData(s) {
	var handle = document.getElementById("statusLine");
	handle.innerHTML = s;
	}	
*/
